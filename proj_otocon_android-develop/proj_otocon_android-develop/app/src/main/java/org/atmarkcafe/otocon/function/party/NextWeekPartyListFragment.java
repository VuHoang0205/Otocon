...
public class NextWeekPartyListFragment extends PartyListFragment {
    @Override
    public void onCreate(@Nullable Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        getArguments().putString(KEY_TITLE, getString(R.string.title_next_week_party_list));
        presenter = new NextWeekPartyPresenter(this);
        setShowIcSearch(getArguments().getBoolean(KEY_SHOW_ADVANCED_SEARCH, true));
    }
}

class NextWeekPartyPresenter extends PartyListPrenter {
    public NextWeekPartyPresenter(PartyListView view) {
        super(view);
    }
    @Override
    public void onExecute(Context context, int action, PartyParams params) {
        view.showProgress(true);
        InteractorManager.getApiInterface(context).nextWeekEvent(params.getPage(),params.getJsonParamsCity(context),
